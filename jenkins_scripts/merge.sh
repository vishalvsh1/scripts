...
git merge -m "Automated merging from jenkins during release from $BRANCH_MERGE_FROM to $BRANCH_MERGE_INTO" $BRANCH_MERGE_FROM
auto_merge=$?
...
then
    ...
    echo "Proceeding with pushing merged changes to branch *$BRANCH_MERGE_INTO"
else
    echo "Auto merge possible? NO. Manual merge required, not prceeding any further."
    exit $auto_merge
fi
